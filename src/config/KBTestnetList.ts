export const KBTestnetList = {
  title: 'pages.knowledge.testnet.explainer-section.blocks.title',
  content: [
    {
      type: 'indexlist',
      value: [
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.intro',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.why-push-exists',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.how-reimagines-interoperability',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.welcome-donut',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.how-to-participate',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.whats-next',
        },
        {
          type: 'text',
          valueKey:
            'pages.knowledge.testnet.explainer-section.blocks.sections.start-building',
        },
      ],
    },
    {
      type: 'list',
      title: 'Quick Links',
      topGap: true,
      bottomGap: true,
      variant: 'row',
      items: [
        {
          slug: 'ecosystem-apps',
          image: '/static/assets/website/chain/knowledge/developer-docs',
          imageSrcSet: false,
          title: 'Ecosystem Apps',
          url: 'http://push.org/ecosystem',
          target: '_blank',
        },
        {
          slug: 'documentation',
          image: '/static/assets/website/chain/knowledge/developer-faucet',
          imageSrcSet: false,
          title: 'Documentation',
          url: 'https://push.org/docs',
          target: '_blank',
        },
        {
          slug: 'knowledge-base',
          image: '/static/assets/website/chain/knowledge/developer-explorer',
          imageSrcSet: false,
          title: 'Knowledge Base',
          url: 'http://push.org/knowledge',
          target: '_blank',
        },
        {
          slug: 'lets-push-yaps',
          image: '/static/assets/website/chain/knowledge/developer-explorer',
          imageSrcSet: false,
          title: `Let's Push Yaps`,
          url: 'http://push.org/lfpush',
          target: '_blank',
        },
        {
          slug: 'push-explorer',
          image: '/static/assets/website/chain/knowledge/developer-explorer',
          imageSrcSet: false,
          title: 'Push Explorer',
          url: 'https://donut.push.network',
          target: '_blank',
        },
        {
          slug: 'push-validator',
          image: '/static/assets/website/chain/knowledge/developer-explorer',
          imageSrcSet: false,
          title: 'Push Validator',
          url: 'https://push.org/docs/chain/node-and-system-tools/running-push-validator/](https://push.org/docs/chain/node-and-system-tools/running-push-validator/',
          target: '_blank',
        },
        {
          slug: 'apply-glassroots',
          image: '/static/assets/website/chain/knowledge/developer-explorer',
          imageSrcSet: false,
          title: 'Apply for Grassroots Program',
          url: 'https://zv9atndluia.typeform.com/to/ItQ7i2sO](https://zv9atndluia.typeform.com/to/ItQ7i2sO',
          target: '_blank',
        },
      ],
    },
  ],
};
