...
Universal transactions and contract-initiated execution share the same purpose which is **executing universal transactions across chains** but serve different actors.
...
The key distinction is that contract-initiated execution is **programmable and autonomous**. Any call that reaches your Push contract can cascade into external chain execution.

Few examples can include: A liquidation trigger, a scheduled job, a governance outcome, or a user action that fans out across chains.

## Key Concepts

### Contract CEA

Every Push Chain smart contract has a deterministically derived **Chain Executor Account (CEA)** on each supported external chain. This is the same concept used for user-initiated transactions, but bound to the contract address instead of a user wallet.
...
UGPC is the on-chain gateway contract on Push Chain through which all outbound cross-chain calls are routed. Your contract calls `UGPC.sendUniversalTxOutbound()`, which relays the payload and / or burns or locks the bridged PRC20 tokens. It then emits the event that the TSS network listens for.
...
| `req.token` | `address` | PRC20 token address on Push Chain to bridge. Use `address(0)` if not bridging tokens. |
| `req.amount` | `uint256` | Amount of PRC20 to bridge. Set to `0` if not bridging. |
| `req.gasLimit` | `uint256` | Gas limit for external-chain execution. Use `0` for the network default. |
| `req.payload` | `bytes` | ABI-encoded calldata for the CEA to execute on the external chain. |
| `req.revertRecipient` | `address` | Address to receive bridged funds if the external transaction reverts. |
...
Only `UNIVERSAL_EXECUTOR_MODULE` is authorized to call this function. Always guard it with an `onlyUniversalExecutor` modifier and track executed `txId`s to prevent replay attacks.
:::
