...
Both universal transactions and contract-initiated execution enable **executing universal transactions across chains**, but they serve different initiators.
...
| _`req.token`_ | `address` | PRC20 token address on Push Chain to bridge. Use `address(0)` if not bridging tokens. |
| _`req.amount`_ | `uint256` | Amount of PRC20 to bridge. Set to `0` if not bridging. |
| _`req.gasLimit`_ | `uint256` | Gas limit for external-chain execution. Use `0` for the network default. |
...
- **CEA funding is your responsibility**<br />
