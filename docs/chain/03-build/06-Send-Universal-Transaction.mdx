...
## Universal Transaction Routes

| **Route** | **Flow** | **Description** |
| --------- | -------- | --------------- |
| Route 1   | Any Origin → Push | Execute directly on Push Chain from any chain including Push. Use a plain address for `tx.to`. |
| Route 2   | Any Origin → External via CEA | Execute on an external chain through your **Chain Executor Account (CEA)**. Set `tx.to` to `{ address, chain }`. |
| Route 3   | External (via CEA) → Push | Execute on Push Chain where the origin execution happens through a CEA on an external chain. Set `tx.from` to `{ chain }`. |
...
// customPropGTagEvent=send_transaction_route2_payload
    ...
    // Route 2: UOA_TO_CEA — Execute a contract call on an external chain (e.g. BNB Testnet)
    // The SDK routes the tx through your CEA (Chain Executor Account) on the target chain.
    import { PushChain } from '@pushchain/core';
...
// customPropGTagEvent=send_transaction_route2_funds
    ...
    // Route 2: UOA_TO_CEA — Transfer native value to an address on an external chain
    import { PushChain } from '@pushchain/core';
    import { createWalletClient, http } from 'viem';
    import { generatePrivateKey, privateKeyToAccount } from 'viem/accounts';
    import * as readline from 'node:readline/promises';
...
// customPropGTagEvent=send_transaction_route3_funds
    ...
    async function main() {
      ...
      await rl.question(':::prompt:::Please send Sepolia ETH to: ' + account.address + ', and ensure your CEA on BNB Testnet holds some USDT. Press Enter to continue.');
