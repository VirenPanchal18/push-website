...
Universal transactions let you execute **native transactions** across Push Chain and supported external chains, whether EVM or non-EVM, through one unified interface. No wrapping, bridging or extra tooling required.
...
| ---------------------- | -------- | ----------------------------------------------------- |
| _`tx.to`_                 | `string` \| `{ address: string; chain: CHAIN }` | Defines the execution target. passing only address triggers Push Chain execution. <br /><br />Passing `{ address, chain }` targets an external chain.                       |
| `tx.from`                 | `{ chain: PushChain.CONSTANTS.CHAIN }` | Optional, when set to an external chain, the transaction is executed via a Chain Executor Account (CEA) on that chain. This is primarily used for external-origin and CEA-based execution flows. <br /><ConstantsDropdown variant="CHAIN" /> |
...
| `tx.data`                 | `string` \| `Array<{ to: string; value: bigint; data: string }>` | The data to send.                                     |
...
| `tx.payGasWith`           | `{ token?: PushChain.CONSTANTS.PAYABLE.TOKEN; slippageBps?: number; minAmountOut?: bigint \| string }` | Pay universal transaction fees using a supported ERC-20 token (e.g., `client.payable.token.USDT`).<br />&nbsp;<br />Optional `slippageBps` (e.g., `100` = 1%) and `minAmountOut` (wei) let you control on-chain swap execution. <ConstantsDropdown variant="PAYABLE_TOKEN" /> |
...
| `SEND-TX-03-01`   | Resolving UEA                       | Resolving Execution Account (UEA) – computing address, checking deployment status/balance | INFO    |
...
<Details summary="Live Playground: Funds Transfer (ERC-20 to External Chain)" className="alert alert--live-play">
...
// customPropGTagEvent=send_transaction_route2_funds_erc20
    ...
    // Route 2: Move ERC-20 to an address on an external chain via CEA
    import { PushChain } from '@pushchain/core';
    import { ethers } from 'ethers';
    import * as readline from 'node:readline/promises';
...
// customPropGTagEvent=send_transaction_route2_funds_with_payload
    ...
    // Route 2: Move ERC-20 to external chain AND atomically call a contract
    import { PushChain } from '@pushchain/core';
    import { ethers } from 'ethers';
    import * as readline from 'node:readline/promises';
...
<Details summary="Live Playground: Bridge Funds Back (ERC-20 from CEA to Push Chain)" className="alert alert--live-play">
...
<Details summary="Live Playground: Funds with Payload (Bridge ERC-20 and Call Contract on Push Chain)" className="alert alert--live-play">
...
// customPropGTagEvent=send_transaction_route3_funds_with_payload
    ...
    // Route 3: CEA_TO_PUSH — Bridge ERC-20 from external chain and call a contract on Push Chain atomically
    import { PushChain } from '@pushchain/core';
    import { ethers } from 'ethers';
    import * as readline from 'node:readline/promises';
