...
Universal transactions let you execute **native transactions** across Push Chain and supported external chains, whether EVM or non-EVM, through one unified interface. No wrapping, manual bridging or extra tooling required.

To understand this concept in details, please see [Understanding Universal Transactions](/docs/chain/build/send-universal-transaction/).
...
**Route 1 → Execute on Push Chain**
  - `to: "0x..."` (plain address)
  - Executes directly on Push Chain from any origin chain

**Route 2 → Execute on External Chain**
  - `to: { address, chain }`
  - Executes on the external chain

**Route 3 → Execute on Push Chain from External Identity**
  - `from: { chain }`
  - Uses your CEA as origin and executes on Push Chain

**Note**: The SDK automatically infers routes from your transaction shape. 
...
| `tx.from`                 | `{ chain: PushChain.CONSTANTS.CHAIN }` | Optional, when set to an external chain, the transaction is executed via a Chain Executor Account (CEA) on that chain. This is primarily used for external origin or CEA-based execution flows. <br /><ConstantsDropdown variant="CHAIN" /> |
| `tx.value`                | `BigInt` | Native value to send, expressed in the smallest unit of the execution context. On Push Chain this is uPC (the smallest unit, like wei in ETH). On external execution routes, this maps to the native asset amount of that route.                           |
| `tx.data`                 | `string` \| `Array<{ to: string; value: bigint; data: string }>` | Use `string` as ABI-encoded calldata for a single call. Use `Array<...>` for batched multicall payload.                                     |
...
| `tx.funds`                | `{ amount: BigInt; token?: PushChain.CONSTANTS.MOVEABLE.TOKEN }` | Moves supported assets from the origin chain to Push Chain within the same call. If `tx.data` is provided, assets are moved and then the contract call is executed atomically. <br /> <ConstantsDropdown variant="MOVEABLE_TOKEN" /> |
...
| `SEND-TX-03-01`   | Resolving UEA                       | Resolving Executor Account (UEA) – computing address, checking deployment status/balance | INFO    |
...
You can pay the origin-chain gas and Push Chain settlement fees using supported ERC-20 tokens (e.g., USDT) instead of the native coin. Use the `payGasWith` field to specify the token to pay fees with.
...
const txResponse = await client.universal.sendTransaction({
  ...
  funds: {
    amount: PushChain.utils.helpers.parseUnits('1', 6), // 1 USDT
    token: PushChain.CONSTANTS.MOVEABLE.TOKEN.ETHEREUM_SEPOLIA.USDT, // move USDT from SEPOLIA ETHEREUM to Push Chain
  },
  payGasWith: {
    token: PushChain.CONSTANTS.PAYABLE.TOKEN.ETHEREUM_SEPOLIA.USDC, // pay gas with USDC instead of native ETH
  },
});
```

Notes:
- Ensure the account holds sufficient balance of the selected `payGasWith.token` on the origin chain to cover gas fees. -->
