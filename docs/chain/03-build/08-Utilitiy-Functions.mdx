...
**_`PushChain.utils.account.deriveExecutorAccount(account, { options? }): Promise<{ address: string, deployed?: boolean }>`_**
...
- **Push account → CEA**: Derive a Chain Executor Account on an external chain from a Push Chain account or UEA
- **Push-native account**: Returns the same account if it's already a Push Chain native account

```typescript
// Derive UEA from UOA (origin account address)
...
import { PushChain } from '@pushchain/core'
    ...
    async function main() {
      // Example 1: Derive UEA from UOA (origin account)
      const uoaAddress = '0xD8d6aF611a17C236b13235B5318508FA61dE3Dba';
      const ueaResult = await PushChain.utils.account.deriveExecutorAccount(uoaAddress);
      console.log('UEA from UOA:');
      console.log(JSON.stringify(ueaResult, null, 2));

      // Example 2: Derive CEA from Push account
      const pushAccount = '0x98cA97d2FB78B3C0597E2F78cd11868cACF423C5';
      const ceaResult = await PushChain.utils.account.deriveExecutorAccount(
        pushAccount,
        { chain: PushChain.CONSTANTS.CHAIN.BNB_TESTNET }
      );
...
import { PushChain } from '@pushchain/core'
    ...
    async function main() {
      ...
      console.log('Controller:', result1.accounts.find(acc => acc.role === 'controller'));
