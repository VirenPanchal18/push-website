...
**_`PushChain.utils.account.convertExecutorToOrigin(account, { options? }): Promise<OriginAccountInfo>`_**

Gives the origin account (UOA) for a given universal executor account (UEA on Push Chain) or a chain executor account (CEA on any external chain). 
