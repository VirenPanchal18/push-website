...
Universal transactions let you send **native** transactions from any Layer 1 chain—EVM or non-EVM, even Push Chain itself. No wrapping, bridging or extra tooling required.
...
| `tx.from`                 | `{ chain: PushChain.CONSTANTS.CHAIN }` | Optional, when set to an external chain, the transaction is executed via a Chain Executor Account (CEA) on that chain. This is primarily used for external-origin and CEA-based execution flows. <br /><br />Use `PushChain.CONSTANTS.CHAIN` values. <ConstantsDropdown variant="CHAIN" /> |
...
<Details summary="Live Playground: Route 2 — Execute on External Chain (Push Chain → BNB / Sepolia / Solana)" className="alert alert--live-play">

<br />

<Tabs className="liveplaytab" groupId="send-universal-route2-examples">

<TabItem value='route2_payload' label='Payload (Contract Call)'>
...
<TabItem value='route2_funds' label='Funds Transfer'>
...
<Details summary="Live Playground: Route 3 — CEA Inbound (External Chain → Push Chain)" className="alert alert--live-play">

<br />

<Tabs className="liveplaytab" groupId="send-universal-route3-examples">

<TabItem value='route3_payload' label='Payload (Contract Call)'>
...
<TabItem value='route3_funds' label='Bridge Funds Back'>
...
<TabItem value='route3_native' label='Bridge Native Back'>
