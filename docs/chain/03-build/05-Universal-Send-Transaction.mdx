...
| --------- | -------- | --------------- |
| Route 1   | External / Push → Push | Execute directly on Push Chain from any chain including Push. Use a plain address for `tx.to`. |
| Route 2   | Push → External (via CEA) | Execute on an external chain through your **Chain Executor Account (CEA)**. Set `tx.to` to `{ address, chain }`. |
| Route 3   | External (via CEA) → Push | Execute on Push Chain from **CEA** on an external chain as the origin. Set `tx.from` to `{ chain }`. |
