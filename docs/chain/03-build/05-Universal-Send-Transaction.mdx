...
| `tx.payGasWith`           | `{ token?: PayableToken; slippageBps?: number; minAmountOut?: bigint \| string }` | Pay universal transaction fees using a supported ERC-20 token (e.g., `client.payable.token.USDT`). Optional `slippageBps` (e.g., `100` = 1%) and `minAmountOut` (wei) let you control on-chain swap execution. |
...
For multicall, the `to` field should always be set to the zero address (`0x0000000000000000000000000000000000000000`). The SDK will `console.warn` if you pass any other address, and this will become mandatory in a future release.
