...
Swap ETH on Sepolia for pSOL on Solana Devnet in a single user signature. Hop 0 bridges 0.001 ETH from the Sepolia EOA into pETH on the UEA on Push Chain, with `value: 5 PC` seeding the UEA's gas via SDK fee-locking. Hops 1 and 2 approve the SwapRouter for pETH and WPC. Hops 3 and 4 swap pETH → WPC → pSOL via two `exactInputSingle` calls on the Push Chain AMM (the AMM has no direct pETH/pSOL pool, so WPC is the intermediate). Hop 5 bridges the pSOL out to the user's Solana Devnet CEA.
...
// customPropGTagEvent=execute_transactions
    ...
    //   Hop 0 (UEA_TO_PUSH)  - Bridge 0.001 ETH from Sepolia EOA → pETH on UEA;
    //                          value=10 PC seeds the UEA with PC for the next four hops.
...
- **Tracking**: Two progress channels work side-by-side and can both be wired at once:
  ...
  - `cascade.wait({ progressHook })` streams `CascadeProgressEvent` per hop (`hopIndex`, `route`, `chain`, `status`, `txHash`, `elapsed`). Best for "X of Y hops confirmed" UIs.
