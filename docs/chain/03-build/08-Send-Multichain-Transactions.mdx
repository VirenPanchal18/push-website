...
> `prepareTransaction` accepts the same transaction arguments as [Send Universal Transaction](./send-universal-transaction). 
...
<Details summary="Live Playground: Execute a Series of Multichain Transactions" className="alert alert--live-play">
...
- [Track Universal Transaction](./track-universal-transaction) to monitor individual transaction confirmation status
- [Contract Initiated Multichain Execution](./contract-initiated-multichain-execution) to trigger multichain flows from on-chain contracts
- [Sign Universal Message](./sign-universal-message) to sign typed messages across chains with a universal signer
- [Utility Functions](./utility-functions) for `encodeTxData`, `parseUnits`, and other helpers used in this page
