...
| `tx.data` | `string` \| `Array<{ to: string; value: bigint; data: string }>` | ABI-encoded calldata (EVM targets), Anchor discriminator + Borsh-encoded instruction data (Solana targets), or an array of calls for multicall mode. Use [`encodeTxData`](/docs/chain/build/utility-functions#encode-transaction-data) to produce the correct bytes for either chain family. |
...
// customPropGTagEvent=execute_transactions_counter
    ...
    const SOL_TEST_PROGRAM = '0x7673075a980bfd5d6b1dffe99c31f63e8938519cc1c2af009dda5e568a94460d'; // Solana Devnet
    ...
    async function main() {
      ...
      // One-time setup for Solana targets — SDK resolves accounts/PDAs/CEA from this IDL
      PushChain.utils.svm.registerIdl(SOL_TEST_PROGRAM, testCounterIdl);
