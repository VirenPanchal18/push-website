...
Multichain transactions let you compose and sequence multiple universal transactions across different routes. 

This allows you to build a **single transaction capable of executing across Push Chain or external chains, or both**, with a single coordinated flow submitted as one user-signed transaction.

**Prerequisite:** Familiarize yourself with [Send Universal Transaction](./send-universal-transaction) before reading this page.
...
| ------------- | -------- | --------------- |
| `tx.to` | `string` \| `{ address: string; chain: CHAIN }` | Target address on Push Chain (plain string), or `{ address, chain }` for an external chain via CEA. <ConstantsDropdown variant="CHAIN" /> |
| `tx.from` | `{ chain: CHAIN }` | Optional. When set, originates from the CEA on that chain (Route 3). |
...
| `tx.gasLimit` | `bigint` | Optional gas limit override. SDK estimates if omitted. |
| `tx.payGasWith` | `{ token?: PayableToken; slippageBps?: number; minAmountOut?: bigint }` | Pay gas with a token instead of native. Use `PushChain.CONSTANTS.PAYABLE.TOKEN.<CHAIN>.<TOKEN>`. |

</PushAPIReference>

**Note**: `PreparedUniversalTx` is primarily an intermediate object passed into `executeTransactions`. Most apps do not need to manually inspect or modify its fields.
...
Executes an array of prepared transactions as a cascade. All hops are composed into a **single user-signed Push Chain transaction**. You sign once, and the SDK relays all subsequent hops automatically.

```typescript
const step1 = await pushChainClient.universal.prepareTransaction({ to: '0xPushChainContract', value: BigInt(0), data: encodedStep1 });
const step2 = await pushChainClient.universal.prepareTransaction({ to: { address: '0xEthContract', chain: PushChain.CONSTANTS.CHAIN.ETHEREUM_SEPOLIA }, value: BigInt(0), data: encodedStep2 });

const cascade = await pushChainClient.universal.executeTransactions([step1, step2]);
```
...
<Details summary="Live Playground: Execute a Cascaded Transaction" className="alert alert--live-play">
...
// customPropGTagEvent=execute_transactions_counter
    ...
    async function main() {
      ...
      await rl.question(':::prompt:::Send at least 0.01 ETH (Sepolia) to: ' + wallet.address + '. Get testnet ETH from https://cloud.google.com/application/web3/faucet/ethereum/sepolia. Press Enter when funded.');
...
// customPropGTagEvent=execute_transactions
    ...
    async function main() {
      ...
      await rl.question(':::prompt:::Send at least 0.002 ETH (Sepolia) to: ' + wallet.address + '. Get testnet ETH from: https://cloud.google.com/application/web3/faucet/ethereum/sepolia. 0.001 ETH is the payload; the rest covers gas on the Sepolia CEA. Press Enter when funded.');
...
// customPropGTagEvent=execute_transactions_batch
    ...
    async function main() {
      ...
      // Hop 2 (Route 2): call Solana Devnet via derived CEA
      const uoa = PushChain.utils.account.toUniversal(wallet.address, { chain: PushChain.CONSTANTS.CHAIN.ETHEREUM_SEPOLIA });
      const solanaCEA = await PushChain.utils.account.deriveExecutorAccount(uoa, {
        chain: PushChain.CONSTANTS.CHAIN.SOLANA_DEVNET,
        skipNetworkCheck: true,
...
- **Single signature**: `executeTransactions` composes all hops into one Push Chain transaction. You sign once and the SDK relays all subsequent hops automatically via the relay network.
...
- [Track Universal Transaction](./track-universal-transaction) — Monitor individual transaction confirmation status
