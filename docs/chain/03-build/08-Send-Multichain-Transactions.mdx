...
// customPropGTagEvent=execute_transactions
    ...
    async function main() {
      ...
      // No 'from' field means Route 1 (UOA initiates). funds.amount tells the
      // SDK to attach 0.001 ETH as msg.value on the source-chain gateway tx,
      // which mints 0.001 pETH on the UEA. value=25 PC bumps the UEA's gas
      // budget — the SDK fee-locks enough ETH from the same UOA tx to seat
      // 25 PC on the UEA for hops 1-5 (2 approves, 2 swaps, 1 outbound).
      const hop0 = await client.universal.prepareTransaction({
        to: client.universal.account,
