...
## Solana (SVM) Program Calls

Solana programs require the client to declare every account an instruction touches. Push Chain handles this for you: pass the Anchor IDL to `encodeTxData` (same way you pass an ABI for EVM), and the SDK resolves program accounts, PDAs, and the sender's CEA automatically when you call `prepareTransaction`. Your Solana calls use the exact same `{ to, value, data }` shape as EVM — no extra IDL parameter on `to`, no separate registration step.

```typescript
// The full JSON emitted by Anchor at target/idl/<program_name>.json.
import testCounterIdl from './target/idl/test_counter.json';

// encodeTxData stashes the IDL internally (keyed by idl.address) so the
// subsequent prepareTransaction / sendTransaction knows how to build the
// Solana payload. You never touch the registry directly.
...
await client.universal.prepareTransaction({
  to: {
    address: '8yNqjrMnFiFbVTVQcKij8tNWWTMdFkrDf9abCGgc2sgx', // base58 or 0x-hex
...
See [Send Multichain Transactions](/docs/chain/build/send-multichain-transactions) for a full end-to-end example.
