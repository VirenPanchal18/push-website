...
**Non-PDA accounts.** If an instruction declares a writable account whose pubkey is neither a PDA (derivable from seeds) nor the sender's CEA, add an `address: "<base58>"` field on that account in the IDL to pin it. Accounts named `cea_authority` (or `cea`, `cea_signer`) are auto-populated with the caller's CEA PDA and do not need pinning.
