...
`encodeTxData` produces chain-appropriate calldata based on the shape of `abi`:

- **EVM ABI array** → 4-byte function selector + ABI-encoded args (viem behavior).
- **Anchor IDL object** → 8-byte Anchor discriminator + Borsh-encoded args, ready to pass as `data` when targeting a Solana chain.

<PushAPIReference>

| **Arguments**     | **Type**          | **Default** | **Description**                                                                                                                                                              |
| ----------------- | ----------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _`abi`_           | `any[] \| Idl`    | -           | Either an EVM ABI array or an Anchor IDL object. The input shape determines which encoding is produced.                                                                      |
...
## Solana (SVM) Utilities

Solana programs require the client to declare all accounts an instruction will touch. Push Chain SDK handles this for you: register your program's Anchor IDL once, and from then on Solana transactions use the same `{ to, value, data }` shape as EVM calls. The SDK reads the IDL to resolve program accounts, PDAs, and the sender's CEA.

{/* API Section Start*/}

### Register Solana Program IDL

**_`PushChain.utils.svm.registerIdl(programAddress, idl): void`_**

```typescript
import testCounterIdl from './test_counter.idl.json';

PushChain.utils.svm.registerIdl(
  '0x7673075a980bfd5d6b1dffe99c31f63e8938519cc1c2af009dda5e568a94460d',
  testCounterIdl
);
```

Call once per program at app startup. Subsequent `prepareTransaction` calls that target the program resolve accounts from this IDL automatically.

<PushAPIReference>

| **Arguments**       | **Type**         | **Description**                                                                                                                |
| ------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| _`programAddress`_  | `0x${string}`    | The Solana program ID as 0x-prefixed 32-byte hex (66 characters total).                                                        |
| _`idl`_             | `Idl` (Anchor)   | The program's Anchor IDL (Anchor ≥ 0.30 spec). Must declare `instructions[].discriminator` and per-account `pda.seeds` / `address` where applicable. |

</PushAPIReference>

**Non-PDA accounts.** If an instruction declares an account whose pubkey is neither a PDA (derivable from seeds) nor the sender's CEA, add an `address: "<base58>"` field on that account in the IDL to pin it. Standard IDLs usually pre-set this for read-only program accounts (e.g. `systemProgram`, `tokenProgram`), so in practice this is mainly something you add for writable custom accounts. Accounts named `cea_authority` (or `cea`, `cea_signer`; camelCase `ceaAuthority` / `ceaSigner` also accepted) are auto-populated with the caller's CEA PDA and do not need pinning.
...
**_`PushChain.utils.svm.getIdl(programAddress): Idl | undefined`_**

Returns the IDL registered for a program, or `undefined` if none has been registered.
...
**_`PushChain.utils.svm.getRegisteredIdls(): Array<{ address, idl }>`_**

Returns every `(programAddress, idl)` pair currently in the registry. Useful for debugging which programs your app has registered.
...
**_`PushChain.utils.tokens.getPRC20Address(token): 0x${string}`_**
...
| **Arguments** | **Type** | **Description** |
| ------------- | -------- | --------------- |
| _`token`_ | `MoveableToken \| { chain: string; address: string }` | Origin token info. Either pass a `MoveableToken` (e.g., from `pushChainClient.moveable.token.*`) or provide the origin chain plus token address. |

</PushAPIReference>

<Details summary="Returns `prc20Address` <string>" className="alert alert--fn-args">
...
> **⚠️ Deprecated**: This function is deprecated and will be removed in v6.0.0. Use [deriveExecutorAccount](#derive-executor-account) instead.
...
Gives the deterministic executor account address for a given origin account (UOA). <br />&nbsp;<br />

Also helps in checking if a given origin account (UOA) has a universal executor account (UEA) deployed on Push Chain or a chain executor account (CEA) deployed on any external chain. <br />&nbsp;<br />
...
|---------------------|-------------|-------------|
| `convertOriginToExecutor` | `deriveExecutorAccount` | • Parameter renamed: `onlyCompute` → `skipNetworkCheck`<br />• Generic `account` parameter accepts string instead of UniversalAccount object<br />• Clearer naming aligned with identity ↔ execution model |
...
Gives the origin account (UOA) for a given Push Chain address. <br />&nbsp;<br />
...
| Deprecated Function | Replacement | Key Changes |
|---------------------|-------------|-------------|
| `convertExecutorToOriginAccount` | `resolveControllerAccount` | • Same functionality with clearer naming<br />• Part of unified identity ↔ execution model |

<PushAPIReference>

| **Arguments**    | **Type**           | **Default** | **Description**                                                                                            |
| ---------------- | ------------------ | ----------- | ---------------------------------------------------------------------------------------------------------- |
| _`pushChainAddress`_      | `string` | -           | Address of the account on Push Chain. |
