...
In this tutorial, you'll build a **Universal Claimable Airdrop** system on Push Chain. This revolutionary approach lets you deploy an airdrop contract **once** and enable users from **any blockchain** to claim their tokens directly from their existing wallets.
...
Traditional airdrops require:
- Deploying contracts on multiple chains
- Managing separate token supplies per chain
- Users bridging tokens or claiming on specific networks
...
- One contract, one token supply, infinite accessibility
...
1. **ERC-20 Token Contract** - The token being airdropped ($UNICORN)
...
This is the **standard OpenZeppelin double-hashing pattern** that matches the `StandardMerkleTree` library used in the frontend.
...
> **Factory Contract:** [0xa82fF9aFd8f496c3d6ac40E2a0F282E47488CFc9](https://donut.push.network/address/0xa82fF9aFd8f496c3d6ac40E2a0F282E47488CFc9?tab=contract)
...
function UniversalAirdropTutorial() {
  ...
  function Component() {
      ...
      try {
        ...
        if (receipt.logs && receipt.logs.length > 0) {
          ...
          if (factoryLog && factoryLog.topics && factoryLog.topics.length > 1) {
            ...
            setError("Failed to parse deployed contract address");
          }
        } else {
    ...
    useEffect(() => {
      const checkClaimerEligibility = async () => {
        if (claimerConnectionStatus === "connected" && claimerPushChainClient?.universal.account && deployedAirdropAddress) {
          const claimerAddress = claimerPushChainClient.universal.account;
    ...
    const claimAirdrop = async () => {
      if (!claimerPushChainClient || !deployedAirdropAddress) {
      ...
      setError("");
      try {
        const claimAddr = claimerPushChainClient.universal.account;
        const entry = convertedAddresses.find(
        ...
        const tx = await claimerPushChainClient.universal.sendTransaction({
          ...
          <div style={{ border: "1px solid #ddd", borderRadius: "12px", padding: "20px", marginBottom: "20px" }}>
            ...
            <div style={{ marginBottom: "20px", backgroundColor: "#fff3e0", borderRadius: "8px", padding: "16px", border: "1px solid #ffe0b2" }}>
              ...
              <div style={{ marginBottom: "12px", display: "flex", justifyContent: "center" }}>
                <PushUniversalAccountButton uid="AirdropClaimer" connectButtonText="Connect Claimer Wallet" />
              </div>
