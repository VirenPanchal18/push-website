...
### 1. True Self-Custody on Every Chain
...
### 2. Deterministic Cross-Chain Identity
...
### 3. Programmable Reach Without Bots
...
### 4. Native `msg.sender` on Every Destination
...
- [Outbound from Push Chain](/docs/chain/build/examples/outbound-from-push-chain): a full reference contract that uses CEAs as the destination identity for outbound dispatches.
- [Inbound to Push Chain](/docs/chain/build/examples/inbound-to-push-chain): the mirror image, an external-chain contract that triggers inbound transactions.
- [Contract-Initiated Multichain Execution](/docs/chain/build/contract-initiated-multichain-execution): covers UGPC, the execution lifecycle, and the security guarantees CEAs underpin.
- [How CEA Works](/docs/chain/deep-dives/how-cea-works): a deep dive on the CEA model.
